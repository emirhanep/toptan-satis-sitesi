        ...
        <div class="right_col" role="main">
          <div class="">
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Ürün <small>Listesi</small></h2>
                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      <thead>
                        ...
                        <th class="text-center align-middle">No</th>
                          ...
                          <th class="text-center align-middle">maliyet </th>
                          <th class="text-center align-middle">Resim</th>
                          <th class="text-center align-middle">İndirimler</th>
...
function addNewSplierSccs(hamSonuc) {
  ...
  for(var i = 0; i < response.length; i++) {
    ...
    '<td class="text-center align-middle"><img src="../productImages/'+response[i]["resim"]+'" width="50px"></td>'+
    '<td class="text-center align-middle"><button type="button" data-idsi="'+response[i]['id']+'" class="btn btn-warning btn-xs indirimBtn">İndirimler</button></td>' +
...
function ilkCalisSccs(hamSonuc) {
  ...
  for(var i = 0; i < response.length; i++) {
    ...
    '<td class="text-center align-middle">'+response[i]["maliyet"]+'</td>'+
    '<td class="text-center align-middle"><img src="../productImages/'+response[i]["resim"]+'" width="50px"></td>'+
    '<td class="text-center align-middle"><button type="button" data-idsi="'+response[i]['id']+'" data-adi="'+response[i]['adi']+'" class="btn btn-warning btn-xs indirimBtn">İndirimler</button></td>' +
    '<td class="text-center align-middle">' +
    '<button type="button" data-idsi="'+response[i]['id']+'" data-adi="'+response[i]['adi']+'" data-maliyet="'+response[i]['maliyet']+'" class="btn btn-primary btn-xs editBtn">Düzenle</button></td><td class="text-center align-middle">'+
    '<button type="button" data-idsi="'+response[i]['id']+'" class="btn '+yazClass+' btn-xs activeBtn">'+yazdeger+'</button></td></tr>';
  $('#datatable-responsive').find('tbody').append(satir);
...
function ilkCalis() {
  ...
  SendPostJson(postData,postUrl,ilkCalisSccs,addNewSplierErr);
      ...
      $(document).ready(function() {
        ...
        $("#newModal .modal-footer").on("click", ".addProduct", function(){    
          $('#errPlace').html('');
          var adi = $('#adi').val();
          var maliyet = $('#maliyet').val();
          var resim = $('#imginpt').val();
        ...
        var modalBdy = '<div id="errPlace" style="width:100%; color:#e74a3b; text-align:center; margin-bottom:20px;"></div>'+  
          '<div class="form-group" style="padding-bottom: 10px">'+
                       ' <label class="control-label col-md-3 col-sm-3 col-xs-12" for="adi">Ürün Adı <span class="required">*</span>' +
                        '</label>' +
                        '<div class="col-md-6 col-sm-6 col-xs-12">' +
                          '<input type="text" id="adi" required="required" class="form-control col-md-7 col-xs-12" style="margin-top:-12px;" value="'+adi+ '">' +
                          '<input type="hidden" id="idbu"  value="'+idsi+ '"></div></div><div class="clearfix"></div>' +
                          '<div class="form-group" style="margin-top: 15px">'+
                       ' <label class="control-label col-md-3 col-sm-3 col-xs-12" for="adi">Ürün Maliyeti <span class="required">*</span>' +
                        '</label>' +
                        '<div class="col-md-6 col-sm-6 col-xs-12">' +
                          '<input type="text" id="maliyet" required="required" class="form-control col-md-7 col-xs-12" style="margin-top:-12px;" value="'+maliyet+ '">' +
                          '</div></div><div class="clearfix"></div>'+
                          '<div class="form-group" style="margin-top: 15px">'+
                       ' <label class="control-label col-md-3 col-sm-3 col-xs-12" for="adi">Ürün Resmi <span class="required">*</span>' +
                        '</label>' +
                        '<div class="col-md-6 col-sm-6 col-xs-12">' +
                        ' <input type="file" id="image-upload" accept="image/*" ><input type="hidden" id="imginpt">' +
                       ' </div></div><div class="clearfix"></div>' +
                       
                       '</div>';

                    var modalNameText = adi + ' Ürün Düzenleme';
                    var btnArry = ["Düzenle","İptal","",""];
                    var btnColorArry = ["btn-primary editProduct","btn-secondary","btn-secondary",""];
        ...
        $(".addNew").on("click",  function(){ 
          ...
          '<div class="form-group" style="padding-bottom: 10px">'+
                       ...
                       '<div class="form-group" style="margin-top: 15px">'+
                       ' <label class="control-label col-md-3 col-sm-3 col-xs-12" for="adi">Ürün Maliyet <span class="required">*</span>' +
                        '</label>' +
                        '<div class="col-md-6 col-sm-6 col-xs-12">' +
                    ...
                    var btnColorArry = ["btn-primary addProduct","btn-secondary","btn-secondary",""];
                    var btnCancel = "modalbttn2";
                    modalPlacement(btnArry,btnColorArry,btnCancel,modalNameText,modalBdy);
        });
        function after(){ }
        $("#newModal .modal-body").on("change", "#image-upload", function(){  
            generalReadImageURLWithSize(this,"imginpt","450","450",after);
         });


        $("#newModal .modal-footer").on("click", ".editProduct", function(){   
          $('#errPlace').html('');
          var adi = $('#adi').val();
          var idbu = $('#idbu').val();
          var maliyet = $('#maliyet').val();
          var imginpt = $('#imginpt').val();
          postData.push({name: "adi", value: adi});
          postData.push({name: "maliyet", value: maliyet});
          postData.push({name: "resim", value: imginpt});
          postData.push({name: "idsi", value: idbu});
         ...
         $("#datatable-responsive").on("click", ".indirimBtn", function(){   
            var adi = $(this).attr("data-adi");
            var idsi = $(this).attr("data-idsi");
            var modalBdy = '<div id="errPlace" style="width:100%; color:#e74a3b; text-align:center; margin-bottom:20px;"></div>'+
          ...
          '<div class="form-group" style="margin-top:15px; padding-bottom: 10px">'+
                       ...
                       ' <label class="control-label col-md-3 col-sm-3 col-xs-12" for="adi">Yüzdelik Oran<span class="required">*</span>' +
                        ...
                        '<div class="col-md-6 col-sm-6 col-xs-12">' +
                          ...
                          '<input type="hidden" id="bag" value="'+idsi+'"><input type="hidden" id="tur" value="1">' +
                       ' </div><div class="clearfix"></div>' +
                       '</div>';
